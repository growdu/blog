...
常规程序都是从main函数开始执行，main函数执行完后程序退出。但vpp利用gcc的 \__attribute__((constructor))特性，使很多函数都在main函数之前执行。主要是插件的配置、注册等操作。当然这些配置和注册操作只是告诉vpp有哪些插件和配置，具体的注册加载执行还需要使用dlopen的方式，按照接口约定拿到具体的执行函数才能进行注册加载以及后续的插件处理。
