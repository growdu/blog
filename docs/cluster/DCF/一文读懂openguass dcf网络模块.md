...
> **reactor主从多线程模型**：Client请求 → Reactor主线程(MainReactor: select + dispatch) → 建立连接 → Acceptor(accept)；同时分发给Reactor子线程(SubReactor: select + dispatch) → 处理请求 → Handler1/Handler2(read + send) → Worker线程池(Worker线程1/线程2: 业务处理)

mec使用tcp监听链路来接收消息，消息接收采用reactor模型实现，最终交给agent来执行。实际流程与上图的reactor主从模型类似，消息接收执行流程如下：

> **mec消息接收执行流程**：左侧acceptor包含三个Socket，通过Pipe (RECV_MODE) `add fd to reactor` 提交给中间reactor pool；reactor pool中三个Reactor负责监听处理事件，再通过 `attach agent` / `dattach agent` 与右侧agent pool通信；每个agent绑定一个job执行具体处理。
...
> **Channel结构**：每个Channel封装两个Pipe，分别为高优先级(high pipe)和低优先级(low pipe)，每个Pipe内部包含socket1（处理send）和socket2（处理recv）；两端节点各通过send/recv连接实现双向通信。
