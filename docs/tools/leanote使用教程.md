...
- 首先先创建一个mongodb.conf的文件，在文件中写入如下内容
  ...
  ```shell
  dbpath=F:\mongodbdata            # 数据库文件
  logpath=F:\mongodblogsmongodb.log    # 日志文件
...
```shell
mongorestore -h localhost -d leanote --dir C:\user1leanotemongodb_backupleanote_install_data
```
导入成功数据后将包含两个默认用户：
