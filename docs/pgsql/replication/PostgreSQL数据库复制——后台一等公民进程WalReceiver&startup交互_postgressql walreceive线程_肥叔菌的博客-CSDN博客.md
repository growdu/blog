...
1. WalReceiver进程进入流复制之前，**startup进程已经recovery.conf文件中的primary_conninfo参数信息解析后填充**到`walrcv->conninfo`、`walrcv->slotname`、`walrcv->receiveStart`、`walrcv->receiveStartTLI`等共享内存WalRcvData中。 WalReceiver进程通过walrcv_connect连接主库。
...
3. (3.1)执行wal_startstreaming开始启动流复制  
   ...
   （3.1.2）如果walrcv_receive获取不到数据，向主库发送write位点、flush位点、apply位点，发送feedback信息(xmin、xmin_epoch、catalog_xmin、catalog_xmin_epoch)，避免vacuum删掉备库正在使用的记录，如果flush了XLOG逻辑位置**唤醒startup进程**，跳转到4  
   （3.1.3）如果walrcv_receive获取EOF，向主库发送write位点、flush位点、apply位点，发送feedback信息(xmin、xmin_epoch、catalog_xmin、catalog_xmin_epoch)，避免vacuum删掉备库正在使用的记录，如果flush了XLOG逻辑位置**唤醒startup进程**，跳转到5  
...
6. 关闭目前的xlog segment文件描述符，发送feedback信息(xmin、xmin_epoch、catalog_xmin、catalog_xmin_epoch)，如果flush了XLOG逻辑位置**唤醒startup进程**，进入步骤7
