...
> **CentOS 9 nginx.conf 注意**：默认 `nginx.conf` 的 `http {}` 块里**没有** `include /etc/nginx/conf.d/*.conf;`，需手动在 `keepalive_timeout` 行之后加一行：
> ```nginx
>     include /etc/nginx/conf.d/*.conf;
> ```
> 否则 `blog.conf` 不会被加载。本机原有的 pgv 配置直接写死在 `nginx.conf` 的 `server {}` 块里，`conf.d/pgv.conf` 是未生效的副本，备份为 `.bak` 即可。
